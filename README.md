...
React 19 + Express 4 + PostgreSQL 16. TypeScript throughout, `strict` everywhere.
...
| **Money in floats** | `0.1 + 0.2`, and a cent goes missing per row | `decimal.js` end to end, `NUMERIC(10,2)` in Postgres, never a JS `number` |
...
A calendar month closes once it is entirely in the past. Employees get `409`
when they try to create, edit or delete anything inside a closed month. Admins
may still correct the data, and every such correction is written to the audit
log.
...
**The `core` layer imports neither Prisma nor Express.** That is not a
convention that erodes over time — it is enforced by
`backend/tests/core.isolation.test.ts`, which fails CI if anything under
`src/core` reaches for infrastructure. The payoff: all the tricky calendar and
money logic is testable without a database.
...
timestamps. There are 10 ordered migrations, including
...
Base path `/api/v1`; `/health` is unversioned. All responses are JSON. Errors
carry both a human-readable `error` and a stable machine `code`:
...
| `POST` | `/shifts` | Both; closed months rejected for employees |
| `PATCH` `DELETE` | `/shifts/:id` | Both, scoped |
| `GET` `POST` | `/sick-days` | Both, scoped |
| `DELETE` | `/sick-days/:id` | Both, scoped |
...
| `ADMIN_LOGIN` / `ADMIN_PASSWORD` | — | **Required.** Seeded admin account |
...
| `CORS_ORIGINS` | `localhost:5173, 127.0.0.1:5173` | Comma-separated allowlist. `*` is rejected — credentialed CORS cannot use it |
...
About 150 tests across four suites, each with a different job:

| Suite | Count | Command | Covers |
|---|---:|---|---|
| Backend unit | 102 | `cd backend && npm test` | Pure domain: Berlin time, balances, pay, CORS parsing, error codes |
| Backend integration | 24 | `cd backend && npm run test:int` | Real Postgres: authorization, overlaps, closed months, terms splitting, audit writes |
...
- **An architecture test.** `core.isolation.test.ts` fails if the domain layer
  starts importing infrastructure.
- **Coverage thresholds are enforced in CI**, not merely reported: 80% lines,
  functions, branches and statements across the domain core and the services.
...
**Writes** to shifts, sick days and terms happen inside transactions, with the
conflict checks and the audit record in the same transaction as the change.
...
│   └── modules/           auth · employees · shifts · sick-days ·
│                          payouts · stats · audit
└── tests/
